    ...
    const handleMobileValidation= (event)=>
  {
    ...
    useEffect(() => {
        if (typeof window !== "undefined" && window.localStorage) {
          if (!(secureLocalStorage.getItem("roles") === null)) {
            const validRole = secureLocalStorage.getItem("roles");
            if (validRole === "ROLE_ADMIN"||validRole === "ROLE_SUPER_ADMIN") {
              fetchData();
    ...
    const fetchData=async(e)=>{
        ...
        setShowData(false);
            try {
          const res = await fetch(`${mainURL}admin/pepole/fetchdata`, {
              method: "GET",
              headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
      ...
      const peopleDataDelete=async(e)=>{
        setDataArrive(false);
        e.preventDefault();
        e.stopPropagation();
        const id=e.target.id;
        try {
            const res = await fetch(`${mainURL}admin/people/delete/${id}`, {
              method: "POST",
              headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
      ...
      const hendleImg = async(e)=>{
        ...
        try {
          const res = await fetch(`${mainURL}download/`+e.target.id, {
            method: "GET",
            headers: {
              Accept: "application/json",
              "Content-Type": "application/json",
  ...
  return (
    ...
    {showData && <Table  hover size="sm" responsive="sm">
        ...
        <tbody  key={data.id}>
         <tr>
            ...
            <td><Button variant="success" size="sm" id={data.id} name={data.id} onClick={openUser} ><i id={data.id} name={data.id} onClick={openUser}  className="bi bi-pencil-square"></i></Button> &ensp;    <Button variant="danger"size="sm" type="submit" id={data.id} name={data.id} onClick={peopleDataDelete} data-toggle="tooltip" data-placement="top" title="Delete"><i id={data.id} name={data.id} onClick={peopleDataDelete} className="bi bi-trash"></i></Button> <Button size="sm" id={data.id} name={data.id} variant='secondary' onClick={hendleImg}><i className="bi bi-eye" id={data.id} name={data.id} onClick={hendleImg}></i></Button></td>
     ...
     <Row className="mb-3">
        ...
        Clear
          ...
          <Col>
                <Button  onClick={Cancel} variant="primary" id="cancelForm" name="cancelForm">Cancel</Button>
                </Col>
        </Row>
    ...
    {showImg && <Container>
      ...
      <Card className='w-50 h-100'>
        ...
        <Card.Footer>
        <Button  onClick={Cancel} variant="primary" id="cancelImg" name="cancelForm">Cancel</Button>
        </Card.Footer>
      </Card>
      </Container>}
    </Container>
